 ...
 * HWPCS. PATHFOLLOWER_QUIET saves 342 bytes of flash memory and 32 bytes of
 * SRAM.
 *
 ...
 * PATHFOLLOWER_REALLY_QUIET additionally saves 234 bytes of flash memory.
 */
 ...
 * (which is internally managed and can be acquired with pathFollower_getStatus())
 ...
 *
 * Use timeTask_captureStartTime(), timeTask_captureStopTime() and
 * timeTask_getDuration() to measure the execution time of this function
 * as it depends heavily on the values given by pathFollower_setSegmentLimit()
 * and pathFollower_setLookaheadDistance(). Note that the first run of
 ...
 * @return  true if the internally managed PathFollowerStatus_t was updated and
 ...
 * The path must consist of at least two points.
 *
...
/**
 * Manually set a command for the path follower (start, pause, reset).
 *
 ...
 *          pathFollower_update().
 */
static inline const PathFollowerStatus_t* __attribute__((always_inline)) pathFollower_getStatus(void) {
	extern PathFollowerStatus_t pathFollower_status;
 ...
 *
 * @return  a pointer to the float containing the lookahead distance in millimeters
 */
static inline const float* __attribute__((always_inline)) pathFollower_getLookaheadDistance(void) {
 ...
 */
static inline uint8_t __attribute__((always_inline)) pathFollower_getSegmentLimit(void) {
	extern uint8_t pathFollower_segmentLimit;
 ...
 * Use timeTask_captureStartTime(), timeTask_captureStopTime() and
 * timeTask_getDuration() to measure the execution time in order to select
 * a feasible value for the segment limit.
 *
 ...
 */
static inline void __attribute__((always_inline)) pathFollower_setSegmentLimit(const uint8_t segmentLimit) {
	extern uint8_t pathFollower_segmentLimit;
