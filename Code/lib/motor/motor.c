...
void Motor_init(void) {
	...
	// since this provides an almost constant relationship between PWM duty cycle
...
__attribute__ ((naked)) void Motor_setVelocityA(const int16_t velocity) {
    asm volatile (
        "sbrc %B[vel], 7"               "\n\t" // if velocity < 0
        "rjmp L_velneg%="               "\n\t" //   then goto L_velneg
        "andi %B[vel], 0x1F"            "\n\t" // velocity & 0x1FFF
        ...
        "sts %B[POCR3B], %B[vel]"       "\n\t" // OCR3B = velocity & 0x1FFF, high byte first
        "sts %A[POCR3B], %A[vel]"       "\n\t" // OCR3B = velocity & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
    "L_velneg%=:"                       "\n\t"
        "neg %B[vel]"                   "\n\t" // two's complement for high byte of velocity
        "neg %A[vel]"                   "\n\t" // two's complement for low byte of velocity
        "sbc %B[vel], __zero_reg__"     "\n\t" // subtract 1 from high byte, if carry flag is set
        "andi %B[vel], 0x1F"            "\n\t" // (-velocity) & 0x1FFF
        "in __tmp_reg__, __SREG__"      "\n\t" // save status register in temporary register
        "cli"                           "\n\t" // disable interrupts
        ...
        "sts %B[POCR3A], %B[vel]"       "\n\t" // OCR3A = (-velocity) & 0x1FFF, high byte first
        "sts %A[POCR3A], %A[vel]"       "\n\t" // OCR3A = (-velocity) & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
        : // no outputs
        : [POCR3A] "n" (& OCR3A),
    ...
    //            OCR3A = (-velocity) & 0x1FFF;
    //        } else {
    //            OCR3A = 0; // OC3A high, AIN1
    //            OCR3B = velocity & 0x1FFF;
    //        }
    //    }
}


__attribute__ ((naked)) void Motor_setVelocityB(const int16_t velocity) {
    asm volatile (
        "sbrc %B[vel], 7"               "\n\t" // if velocity < 0
        "rjmp L_velneg%="               "\n\t" //   then goto L_velneg
        "andi %B[vel], 0x1F"            "\n\t" // velocity & 0x1FFF
        ...
        "sts %B[POCR4B], %B[vel]"       "\n\t" // OCR4B = velocity & 0x1FFF, high byte first
        "sts %A[POCR4B], %A[vel]"       "\n\t" // OCR4B = velocity & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
    "L_velneg%=:"                       "\n\t"
        "neg %B[vel]"                   "\n\t" // two's complement for high byte of velocity
        ...
        "sts %B[POCR4A], %B[vel]"       "\n\t" // OCR4A = (-velocity) & 0x1FFF, high byte first
        "sts %A[POCR4A], %A[vel]"       "\n\t" // OCR4A = (-velocity) & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
        : // no outputs
        : [POCR4A] "n" (& OCR4A),
    ...
    //            OCR4A = (-velocity) & 0x1FFF;
    //        } else {
    //            OCR4A = 0; // OC4A high, BIN2
    //            OCR4B = velocity & 0x1FFF;
    //        }
    //    }
}


__attribute__ ((naked)) void Motor_setVelocities(const int16_t velocityA, const int16_t velocityB) {
    asm volatile (
        "sbrc %B[velA], 7"              "\n\t" // if velocityA < 0
        "rjmp L_velAneg%="              "\n\t" //    then goto L_velAneg
        "andi %B[velA], 0x1F"           "\n\t" // velocityA & 0x1FFF
        ...
        "sts %B[POCR3A], __zero_reg__"  "\n\t" // OCR3AH = 0, high byte first (OC3A high, AIN1)
        "sts %A[POCR3A], __zero_reg__"  "\n\t" // OCR3AL = 0, then low byte (OC3A high, AIN1)
        "sts %B[POCR3B], %B[velA]"      "\n\t" // OCR3B = velocityA & 0x1FFF, high byte first
        "sts %A[POCR3B], %A[velA]"      "\n\t" // OCR3B = velocityA & 0x1FFF, then low byte
        "sbrc %B[velB], 7"              "\n\t" // if velocityB < 0
        "rjmp L_velBneg%="              "\n\t" //    then goto L_velBneg
    "L_velBpos%=:"                      "\n\t"
        "andi %B[velB], 0x1F"           "\n\t" // velocityB & 0x1FFF
        "sts %B[POCR4A], __zero_reg__"  "\n\t" // OCR4AH = 0, high byte first (OC4A high, BIN2)
        "sts %A[POCR4A], __zero_reg__"  "\n\t" // OCR4AL = 0, then low byte (OC4A high, BIN2)
        "sts %B[POCR4B], %B[velB]"      "\n\t" // OCR4B = velocityB & 0x1FFF, high byte first
        "sts %A[POCR4B], %A[velB]"      "\n\t" // OCR4B = velocityB & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
    "L_velAneg%=:"                      "\n\t"
        "neg %B[velA]"                  "\n\t" // two's complement for high byte of velocityA
        "neg %A[velA]"                  "\n\t" // two's complement for low byte of velocityA
        "sbc %B[velA], __zero_reg__"    "\n\t" // subtract 1 from high byte, if carry flag is set
        "andi %B[velA], 0x1F"           "\n\t" // (-velocityA) & 0x1FFF
        "in __tmp_reg__, __SREG__"      "\n\t" // save status register in temporary register
        "cli"                           "\n\t" // disable interrupts
        "sts %B[POCR3B], __zero_reg__"  "\n\t" // OCR3BH = 0, high byte first (OC3B high, AIN2)
        "sts %A[POCR3B], __zero_reg__"  "\n\t" // OCR3BL = 0, then low byte (OC3B high, AIN2)
        "sts %B[POCR3A], %B[velA]"      "\n\t" // OCR3A = (-velocityA) & 0x1FFF, high byte first
        "sts %A[POCR3A], %A[velA]"      "\n\t" // OCR3A = (-velocityA) & 0x1FFF, then low byte
        "sbrs %B[velB], 7"              "\n\t" // if velocityB >= 0
        "rjmp L_velBpos%="              "\n\t" //    then goto L_velBpos
    "L_velBneg%=:"                      "\n\t"
        "neg %B[velB]"                  "\n\t" // two's complement for high byte of velocityB
        "neg %A[velB]"                  "\n\t" // two's complement for low byte of velocityB
        "sbc %B[velB], __zero_reg__"    "\n\t" // subtract 1 from high byte, if carry flag is set
        "andi %B[velB], 0x1F"           "\n\t" // (-velocityB) & 0x1FFF
        "sts %B[POCR4B], __zero_reg__"  "\n\t" // OCR4BH = 0, high byte first (OC4B high, BIN1)
        "sts %A[POCR4B], __zero_reg__"  "\n\t" // OCR4BL = 0, then low byte (OC4B high, BIN1)
        "sts %B[POCR4A], %B[velB]"      "\n\t" // OCR4A = (-velocityB) & 0x1FFF, high byte first
        "sts %A[POCR4A], %A[velB]"      "\n\t" // OCR4A = (-velocityB) & 0x1FFF, then low byte
        "out __SREG__, __tmp_reg__"     "\n\t" // restore status register
        "ret"                           "\n\t" // return
        : // no outputs
        : [POCR3A] "n" (& OCR3A),
    ...
    //        if (velocityA < 0) {
    //            OCR3B = 0; // OC3B high, AIN2
    //            OCR3A = (-velocityA) & 0x1FFF;
    //        } else {
    //            OCR3A = 0; // OC3A high, AIN1
    //            OCR3B = velocityA & 0x1FFF;
    //        }
    //
    //        if (velocityB < 0) {
    //            OCR4B = 0; // OC4B high, BIN1
    //            OCR4A = (-velocityB) & 0x1FFF;
    //        } else {
    //            OCR4A = 0; // OC4A high, BIN2
    //            OCR4B = velocityB & 0x1FFF;
    //        }
    //    }
}
